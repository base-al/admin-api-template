...
// OLTConfig holds OLT configuration settings
// DEPRECATED: OLT configuration is now managed through the database settings system.
// This struct is kept for backward compatibility only.
type OLTConfig struct {
	OLTServiceConfig OLTServiceConfig `json:"olt_service_config"`
}

// OLTServiceConfig holds configuration for the OLT service
// DEPRECATED: OLT configuration is now managed through the database settings system.
// Configure OLT settings through the admin web interface at /app/settings/olt
type OLTServiceConfig struct {
	SmartOLTAPIURL    string `json:"smart_olt_api_url"`
	SmartOLTAPIKey    string `json:"smart_olt_api_key"`
	SNMPCommunity     string `json:"snmp_community"`
	SNMPVersion       string `json:"snmp_version"`
	SNMPPort          int    `json:"snmp_port"`
	SNMPTimeout       int    `json:"snmp_timeout"`
	SNMPRetries       int    `json:"snmp_retries"`
	HuaweiSSHUsername string `json:"huawei_ssh_username"`
	HuaweiSSHPassword string `json:"huawei_ssh_password"`
	HuaweiSSHPort     int    `json:"huawei_ssh_port"`
	HuaweiSSHTimeout  int    `json:"huawei_ssh_timeout"`
}
...
func parseMiddlewareConfig(config *Config) {
	...
	config.Middleware = MiddlewareConfig{
		...
		AuthSkipPaths:      parsePathList("MIDDLEWARE_AUTH_SKIP_PATHS", "/api/auth/login,/api/auth/register,/api/auth/forgot-password,/docs,/docs/,/swagger,/swagger/,/api/search"),
...
func (c *Config) GetInt(key string, defaultValue int) int {
	valueStr := c.GetString(key, fmt.Sprintf("%d", defaultValue))
	value, err := strconv.Atoi(valueStr)
	if err != nil {
		logConfigError("Invalid %s value: %s. Using default: %d", key, valueStr, defaultValue)
		return defaultValue
